...
window.onload = function (){
    for (let i = current; i< 10;i++){
        div.innerText+=locations[i].title + ' ' +locations[i].type + '\n';
    }
    current+=10;
}
...
prevB.onclick = function (){
    if(current>=10) {
        ...
        for (let i = current; i < current+10 && i < locations.length; i++) {
            div.innerText += locations[i].title + ' ' + locations[i].type + '\n';
        }
    }
    else{
        div.innerText = 'You cannot return further';
    }
}
...
nextB.onclick=function (){
    if (current<locations.length){
        ...
        div.innerText = 'You cannot move further';
    }
}